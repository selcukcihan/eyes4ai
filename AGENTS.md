...
- During the current development phase, do not preserve backwards compatibility for local telemetry formats or dev-only data. If a schema or storage change is cleaner, prefer wiping and regenerating `.eyes4ai/private/` data over adding migration complexity.
